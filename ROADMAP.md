...
> **This is the authoritative ordering of the work.** `PROGRESS.md` holds
> current state and findings; `TASK-*.md` hold the per-task detail; this file
> holds the plan. Written 2026-08-10, after the scope reset (`PROGRESS.md` §2).
...
Two operator decisions shape this plan (`PROGRESS.md` §2):
...
3. **Replaces the messy 3.8.4-from-git install** with a real, package-based
   ...
   (`PROGRESS.md` §2.3) without an in-place upgrade.
4. **Hardware-validates the stock→Neptune conversion** (`PROGRESS.md` §5.4):
   on the fresh install, `omarchy-deck-kernel.sh` finally runs its real
...
- [ ] T4's scope known — `FINDING-T2-gamepad-spike.md` written
...
short, targeted Deck iterations over `deck-sync.sh`.

| # | Item | Where | Task ref |
|---|---|---|---|
| P2.1 | Desktop-mode input mapper as a `--user` service + `squeekboard`, on the Deck; verify against 4.0's `uwsm` targets | Deck (short) | `TASK-T3` §4 |
| P2.2 | Hardware parity batch 1: Wi-Fi, BT, audio, trackpads/gyro, buttons in both sessions → `FINDING-hardware-parity.md` | Deck | `TASK-T3` §5 |
| P2.3 | Hardware parity batch 2: TDP, fan, battery — **operator approval per item, every time** | Deck | `TASK-T3` §5 |
| P2.4 | Shell integration on Quickshell (now testable): pin the return icon, QAM/Power-menu trigger placement | Deck (short) | `TASK-T3` §6 |
| P2.5 | T4: the 8 installer screens, per T2's finding; rotation handling per P1.5's recon | QEMU | `TASK-T4` |
| P2.6 | T4: full install flow in QEMU with virtual gamepad only, no keyboard | QEMU | `TASK-T4` |
| P2.7 | T5: fork `omarchy-iso`; Deck pacman package + `pre-refresh-pacman.d/` hook; Valve repos into the build; live-image firmware per §5.1's answer; `lib32-vulkan-radeon` pinned | Dev | `TASK-T5` |
| P2.8 | T5: script-override loader wired; CI builds the ISO on tag; size recorded | Dev + CI | `TASK-T5` |
...
| # | Item | Task ref |
|---|---|---|
| P3.1 | Factory-reset the Deck via Valve's recovery image; **document the recovery path while doing it** (this becomes the README section) | `TASK-T6` §4 |
| P3.2 | Full hardware matrix, one run, in order: Ventoy boot → controller-only install (no keyboard attached, Wi-Fi joined on-screen) → first boot lands in Gaming Mode → Steam signs in → hardware works → Desktop Mode → back → reboots persist | `TASK-T6` §2 |
| P3.3 | Kernel-update resilience: force a kernel reinstall, reboot, Neptune entry still default | `TASK-T6` §3 |
| P3.4 | Fix → re-run the affected portion; be honest about blast radius | `TASK-T6` §2 |
| P3.5 | Trademark/glyph check (Valve iconography), honest LCD statement, known-issues list | `TASK-T6` §5, `PROGRESS.md` §5.8 |
| P3.6 | Rebase onto Omarchy 4.0 **stable** (whenever it lands — if it lands earlier, fold in during phase 2), re-verify the shell hooks | `TASK-T6` §1 |
| P3.7 | Tag, build, checksum, publish | `TASK-T6` §7 |
...
|---|---|---|
| Live ISO can't drive the OLED radio | 1 | Firmware into the live image (`TASK-T5` §2); worst case partially reopens the offline-mirror question |
...
| Omarchy 4.0 beta churn breaks shell hooks | 2 | Keep integration points thin (`PLAN.md` §11); re-verify at P3.6 |

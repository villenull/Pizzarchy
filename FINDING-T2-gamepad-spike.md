...
(Wi-Fi password, account credentials). That is `PLAN.md` §6.1a items 5 and 7 —
not a general UI problem.

Decided 2026-08-10, in QEMU (`vm-gamepad-spike-test.sh`), against the
...
`TASK-T2` framed the two outcomes as differing by weeks:
...
`PLAN.md` §6.1's "if a generic mapper works, it solves navigation for
...
`deck-input-mapper.py` — reads a pad's evdev node, emits keyboard events
...
`test-deck-input-mapper.py` — 26 assertions on the pure translation core. No
...
`vm-gamepad-spike-test.sh` boots the substrate and, **inside the guest**,
...
(exit status, the file the prompt wrote), per `PLAN.md` §9.7's
...
- **Wi-Fi password** (`PLAN.md` §6.1a item 7) — on the critical path since
...
- Keep `PLAN.md` §6.1a's aggressive prompt reduction. Every prompt is a
...
- Glyphs (`PLAN.md` §6.1 A/B/X/Y iconography) remain open and are a
  trademark question, not an engineering one (`PROGRESS.md` §5.5).

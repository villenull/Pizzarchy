...
> reversed** — see `PROGRESS.md` §2.4. This project ships its own session
> layer (`deck-session.sh`).
>
...
1. ~~**Pivot to Omarchy 3.8.4** as v0's target, not Quattro 4.x.~~
   **Reversed:** the target is Omarchy 4.0 (`PROGRESS.md` §2.3), and the
   "v0" framing itself is gone (`PROGRESS.md` §2.1). The underlying
...
2. ~~**DeckShift owns session switching.**~~ **Reversed** (`PROGRESS.md`
   §2.4). `deck-session.sh` owns it. The `steamos-session-select` shim
...
1. **Pivot to Omarchy 3.8.4** as v0's target, not Quattro 4.x. The Deck runs
   ...
   `PROGRESS.md`'s "which Omarchy" question outright — T3's
   shell integration is now testable on the only test asset.
2. **DeckShift owns session switching.** `deck-session.sh` is retired as a
...
affordance silently does nothing — `PLAN.md` §8.1's failure mode, in the one
...
- **`git revert bcdab80` would be wrong** — it deletes the source of the
  surviving `steamos-session-select` shim. `deck-session.sh` needs hand-editing
...
`deck-session.sh`'s drop-in never won, on any machine. Its own comment claimed

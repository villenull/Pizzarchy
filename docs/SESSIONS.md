...
2. **Context size per request.** Every message re-sends the accumulated
   context. A session that has read `docs/PLAN.md` (~13k tokens) plus a dozen
...
5. **Re-deriving things.** Every time a session re-discovers a finding
   because it wasn't written to `docs/PROGRESS.md`, that's pure waste.
...
3. **Never read `docs/PLAN.md` whole after session 1.** It's the largest file
   here. Task files are written to be self-contained; when one cites a
   `docs/PLAN.md` section, read *that section*, not the file.
...
5. **Write findings down immediately.** `docs/PROGRESS.md` is the only thing
...
**Blocks 1–9 are done.** Nine sessions consumed; see `docs/PROGRESS.md` §8.

| # | Block | Task | Model | Notes |
|---|---|---|---|---|
| 1–2 | ✅ Test infrastructure | T0 | Sonnet | QEMU harness, `tools/deck-sync.sh`, CI, shellcheck |
...
### Remaining — see `docs/ROADMAP.md` for the full phase plan
...
| P1.3 | ✅ done — `docs/findings/T2-gamepad-spike.md`; T4 is days not weeks. OSK choice deferred to the real ISO | Sonnet | |
...
| P3.1–7 | Factory reset → full matrix → resilience → release | **Opus** | **Hardware.** See `docs/ROADMAP.md` phase 3 |
...
1. Update `docs/PROGRESS.md`: status, findings, blocked-on-human, next step
2. Commit
3. `/clear`

If `/usage` shows you're near the window limit mid-block: **stop at the
next clean boundary, write `docs/PROGRESS.md`, commit.** A half-finished block

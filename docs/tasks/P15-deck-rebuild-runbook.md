...
(`docs/ROADMAP.md` phase 1): the live-ISO Wi-Fi question (§5.1, top unknown),
...
every command itself: recon, the installer prep, `src/omarchy-deck-kernel.sh`,
`src/deck-session.sh`, journal reads, reboots. The operator watches.
...
3. **Agent takes over via `ssh root@<ip>`.** Recon checklist, all read-only:
   ...
   - **Input:** `cat /proc/bus/input/devices` — what the controller looks
     ...
     directly** (`docs/findings/T2-gamepad-spike.md`): the spike used a virtual pad
   ...
   - **Try the mapper for real, right there:** copy `src/deck-input-mapper.py`
...
8. Agent, dev machine: `~/.ssh/config` Host alias (`steamdeck` → the IP), so
   `tools/deck-sync.sh` works with its defaults; verify `ssh steamdeck true`.
   **This also closes T0's last gap: `tools/deck-sync.sh` runs against real
...
10. `git clone` the repo onto the Deck (or `tools/deck-sync.sh` it over).
...
14. `./src/deck-session.sh` (installs everything except the default flip), then
    ...
    - Only after both work: `./src/deck-session.sh stage-default-session`,
      reboot, *Expect:* boots to Gaming Mode unattended.
15. Final snapshot + `docs/PROGRESS.md` updates + commit from the Deck clone or
    the dev machine.
...
- Truly bricked-looking → recovery USB → stock SteamOS → decide fresh.
  **This outcome is acceptable by decision** (`docs/PROGRESS.md` §2.5); it costs
...
- [ ] `tools/deck-sync.sh` proven against real hardware — T0's last gap
- [ ] Both session-switch directions proven without DeckShift
- [ ] `docs/PROGRESS.md` §5.1/§5.2/§5.3/§5.4 + T3 status updated, committed, PR'd

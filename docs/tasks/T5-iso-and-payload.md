...
| Requirement | Why |
|---|---|
| ~~`~/.config/autostart/steam.desktop`~~ **CANCELLED by R-42, and closed by measurement in R-53.** Steam cannot drive a Wayland desktop: XTEST reaches X11 clients under XWayland but **neither Wayland-native clients nor the compositor's pointer** (measured 2026-08-11, `docs/findings/P20-steam-xtest-closure.md`), and Steam creates **no** virtual keyboard or mouse — only the gamepad. Desktop Mode uses our own layer-shell OSK, with squeekboard as the automatic fallback. 🔴 **CORRECTED 2026-08-11:** this row previously said *"a resident Steam removes lizard mode's mouse/keyboard nodes"* — **R-41 retracted that mechanism itself.** The nodes are all **present and silent**; Steam consumes the controller over **hidraw**, invisible to evdev (`P17-input-and-osk.md:466-489`). The row also still said "Resolve before shipping" while its own header said CANCELLED. Kept for history: Valve's keyboard lives inside the Steam client, `-silent` starts it resident with zero windows, and a resident Steam would additionally need a **StatusNotifier tray host**. None of that is actionable now |

...
> `tools/deck-sync.sh` never run against real hardware (`docs/PROGRESS.md` §5.7).
...
iteration would dominate total build time. See `docs/PLAN.md` §9 for the full
...
- **Assert on artifacts, not log text.** Log-scraping would have passed on
  the `docs/PLAN.md` §8.1 bug (script printed success while doing nothing).
...
- Write `docs/findings/testing-usb.md` documenting: install Ventoy once on the test
...
- This is a human-executed setup step. Document it clearly, list it under
  "Blocked on human" in `docs/PROGRESS.md`, and do not attempt it yourself.
...
- Write `tools/deck-sync.sh`: rsync changed scripts to the Deck over SSH,
...
- Also write `tools/deck-snapshot.sh` / `tools/deck-rollback.sh` wrapping btrfs
...
- `shellcheck` across all shell in the repo, including
  `src/omarchy-deck-kernel.sh`. Fix what it flags there — it was written
...
- [ ] `docs/findings/testing-usb.md` exists and is specific enough to follow without
      further explanation
- [ ] Script-override loader implemented and unit-tested with fixtures
- [ ] `tools/deck-sync.sh` written (untested against real hardware is OK

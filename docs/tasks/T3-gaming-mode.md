...
**Status: in progress.** The session layer exists (`src/deck-session.sh`); Gaming
...
- T0 done — `tools/deck-sync.sh` is where this task's iteration speed comes from
- T1 done — Neptune kernel booting

## ⚠️ The starting point in `docs/PLAN.md` §6.4 is obsolete — read this instead

`docs/PLAN.md` §6.4 and §5 say to fork `28allday/Super-Shift-S-Omarchy-Deck-Mode`
...
the *switch*. See `docs/PROGRESS.md` §4 and `docs/findings/deckshift-hybrid.md`.

Read DeckShift as a reference design if useful. Do not vendor it.

## Steps

### 1. ✅ The switch layer — `src/deck-session.sh`

Done, with two bugs from its first version fixed (`docs/PROGRESS.md` §4.2). Four
...
Without it, Steam's own affordance **silently does nothing** — `docs/PLAN.md` §8.1's
...
files — inventory in `docs/findings/deckshift-hybrid.md`). **The phase-1 rebuild
(`docs/ROADMAP.md` P1.5) wipes all of it** — no manual unwind, and the `/tmp`
backups stop mattering (`docs/PROGRESS.md` §2.5).
...
preconditions are hardware-proven (`docs/findings/R1-10.3.md`). Two traps already
...
Work `docs/PLAN.md` §6.5's parity table. Iterate with `tools/deck-sync.sh` — **never
...
| Wi-Fi | OLED radio differs from LCD. See also `docs/PROGRESS.md` §5.1 — the *live ISO* case is separate and unverified |
...
Record pass/fail plus the fix in `docs/findings/hardware-parity.md`. That doc is the
LCD-support roadmap later.

### 6. Shell integration — Omarchy 4.0

Deferred until there is a 4.0 test target (`docs/PROGRESS.md` §2.3, §6). Two items,
...
- [ ] `docs/findings/hardware-parity.md` complete for OLED, LCD rows marked untested

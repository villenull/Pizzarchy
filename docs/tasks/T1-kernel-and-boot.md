...
> run their no-op path on the real Deck (`docs/PROGRESS.md` §5.2). Closes by design
> in `docs/ROADMAP.md` P1.5.
>
> Steps 7–8 were completed 2026-08-10 (`test/vm/vm-default-entry-test.sh`, all four
...
> ESP's *mount options*). Use `./src/omarchy-deck-kernel.sh list-stages`.
...
Every bug in `docs/PLAN.md` §8 lives here. Silent failures, wrong ESP paths,
...
- Read `docs/PLAN.md` §6.3, §8 (all), §11

## Starting point

`src/omarchy-deck-kernel.sh` already exists in this repo. It was written
...
filename, UKI filename, Limine entry path. Per `docs/PLAN.md` §11 this is a real
...
### 3. The pacman hook (`docs/PLAN.md` §11)
...
`docs/PLAN.md` §8.5 this is a **security-relevant global loosening applied to fix
...
- Keep the workaround for now, but **document the tradeoff visibly** in the
  script and in `docs/findings/esp-permissions.md` — don't bury it.

### 6. Split into stages

For T0's `tools/deck-sync.sh` loop to be useful, each stage must be independently
runnable:
...
Settle it on the `test/images/vm-neptune-image.sh` substrate, not on hardware: set
...
- [x] **Deliberate-failure test** — three of them, in `test/vm/vm-default-entry-test.sh`:
...
- [ ] **Stock→Neptune conversion validated on hardware** (`docs/PROGRESS.md` §5.2,
      `docs/ROADMAP.md` P1.5)

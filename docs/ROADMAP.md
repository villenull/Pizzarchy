...
| P2.4b 🆕 | **T8: the on-screen keyboard we draw ourselves** — split layout, **two cursors** (one per trackpad), one core with a TTY renderer for the installer and a layer-shell renderer for Desktop Mode. squeekboard cannot do dual-cursor selection in any configuration, and the live ISO has no compositor at all (T2 §4). The mapper already owns the input half and the STEAM+X chord | Dev + Deck | `docs/tasks/T8-onscreen-keyboard.md` |

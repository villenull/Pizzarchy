...
| P3.2 🔴 | Full hardware matrix, one run, in order: Ventoy boot → controller-only install (no keyboard attached, Wi-Fi joined on-screen) → first boot lands in Gaming Mode → Steam signs in → hardware works → Desktop Mode → back → reboots persist. **⚠️ BLOCKED on the "no keyboard attached" clause by a P1 found on the first hardware boot (2026-08-15, session 28): the OSK mapper + `python-evdev` are not in the live ISO, so every text-entry screen has no on-screen keyboard — `docs/findings/P32-osk-mapper-missing-from-live-iso.md`. The rest of the matrix is being run this session with a USB keyboard attached; the controller-only clause reopens until the mapper ships in the ISO and is re-verified by live boot.** | `docs/tasks/T6-integration-release.md` §2 |
...
| P3.6 🟢 | Rebase onto Omarchy 4.0 **stable** — **DEV-SIDE COMPLETE 2026-08-15 (session 27), merged to `main`**. Stable shipped 2026-08-14 (`v4.0.0` = `f0020448`); pin measured from inside the ISO, 127-commit delta classified (1 BREAKS-US, self-healed by the submodule bump; boot/lock/menu safe), and **a real ISO built and verified**: `omarchy-2026.08.15-x86_64-quattro.iso`, 6.38 GiB, sha256 `e9fbd8ed…68c5`, all eight guards green. Path B on package naming (the stable channel's `omarchy-dev` IS the release commit, so guard 6.4b stands). **§14.6 root-caused and fixed** — Valve's gamescope is what ships the session file, and nothing was installing it. **Remaining: the operator-present Deck update** (`docs/tasks/P36-deck-stable-update-runbook.md`) and proving the ISO installs (P3.2). Findings: `T9-stable-pin.md`, `T9-stable-delta-classification.md`, `T9-stable-rebase-remaining.md`. **✅ CLOSED 2026-08-15 (session 28) BY A ROUTE THE RUNBOOK DID NOT ANTICIPATE: the Deck was *reinstalled from our own stable ISO*, not updated in place.** `omarchy-version` is `4.0.0.r1744.gf002044-1` = `iso/RUNTIME`'s pin, read off the device. The in-place runbook was therefore never executed and is now **superseded** — keep it only as the recipe for updating an *existing* install; it is not the record of how this Deck got here. Per its own §"Why in-place update, not reinstall": phase 3's install **is** now the first from this ISO, and it exposed four P1s (`docs/PROGRESS.md` §5.32) | `docs/tasks/T6-integration-release.md` §1 |

...
| P2.4b 🟡 | **T8: the on-screen keyboard we draw ourselves** — split layout, **two cursors** (one per trackpad), one core with a TTY renderer for the installer and a layer-shell renderer for Desktop Mode. squeekboard cannot do dual-cursor selection in any configuration, and the live ISO has no compositor at all (T2 §4). **Steps 1–4 done (session 18):** `src/deck_osk_layout.py` (layout, hit-testing, shift layers, **two absolute cursors**), the mapper declaring every character keycode, and `--type` for renderer-free verification — the TTY renderer, and `--osk-backend=tty` in the mapper — 305 unit assertions plus a 19-assertion end-to-end drive through a virtual pad, **46/46 mutations caught**, **not yet run on the Deck**. **Next: step 5, the layer-shell renderer** for Desktop Mode. Also owed: the keyboard and `gum` sharing one console in QEMU | Dev + Deck | `docs/tasks/T8-onscreen-keyboard.md` |

...
| P2.4b 🟡 | **T8: the on-screen keyboard we draw ourselves** — split layout, **two cursors** (one per trackpad), one core with a TTY renderer for the installer and a layer-shell renderer for Desktop Mode. squeekboard cannot do dual-cursor selection in any configuration, and the live ISO has no compositor at all (T2 §4). **Steps 1–3 done (session 18):** `src/deck_osk_layout.py` (layout, hit-testing, shift layers, **two absolute cursors**), the mapper declaring every character keycode, and `--type` for renderer-free verification — 134 + 89 + 12 assertions, **23/23 mutations caught**, **not yet run on the Deck**. **Next: step 4, the TTY renderer** — QEMU with a virtual pad, no hardware. Steps 4–5 (the two renderers) are the remaining bulk | Dev + Deck | `docs/tasks/T8-onscreen-keyboard.md` |

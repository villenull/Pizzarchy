...
| P2.4b 🟡 | **T8: the on-screen keyboard we draw ourselves** — split layout, **two cursors** (one per trackpad), one core with a TTY renderer for the installer and a layer-shell renderer for Desktop Mode. squeekboard cannot do dual-cursor selection in any configuration, and the live ISO has no compositor at all (T2 §4). **Steps 1–2 done (session 18):** `src/deck_osk_layout.py`, the mapper declaring every character keycode, and `--type` for renderer-free verification — 100 + 12 new assertions, 12/12 mutations caught, **not yet run on the Deck**. **Next: step 3, the absolute dual-cursor mapping** — needs no hardware. Steps 4–5 (the two renderers) are the remaining bulk | Dev + Deck | `docs/tasks/T8-onscreen-keyboard.md` |

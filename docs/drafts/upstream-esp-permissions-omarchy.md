...
report, written for `docs/PLAN.md` §8.5 / `docs/tasks/T1-kernel-and-boot.md` step 5.
...
defaults. See `docs/PROGRESS.md` (Findings, the paragraph beginning "PLAN.md
...
- Propose the cleaner fix: elevate the specific existence/read check
  ...
  `docs/findings/esp-permissions.md`.
...
- Do **not** propose that Omarchy adopt this project's own workaround (the
  ...
  `src/omarchy-deck-kernel.sh`'s `stage_esp_permissions`) as the upstream fix.

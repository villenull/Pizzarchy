...
> ## Where things stand (updated 2026-08-10, end of session 14)
>
> ### ✅ PHASE 1 IS COMPLETE. The project is in phase 2.
>
> **T0, R1, T1, T2 are done. T3 is partway.** P1.5 — the big Deck
> recon/rebuild session — ran end to end and met every phase-1 exit criterion.
>
> **The Deck is now a real test bed for the first time:** package-based
> **Omarchy 4.0 + Neptune 6.11.11-valve29**, unencrypted, booting unattended,
> both session directions working, zero DeckShift. Reach it with
> **`ssh steamdeck`** (key-based, passwordless sudo). Snapshots #1/#2/#3 =
> pre-Neptune / post-conversion / complete.
>
...
>    the wrong thing
> 2. `docs/PROGRESS.md` **§5.9–§5.14** — the six issues P1.5 *opened*. Several
>    change T3/T4/T5, and two would ship as defects if ignored
> 3. `docs/ROADMAP.md` phase 2 — the work queue from here
> 4. `docs/PROGRESS.md` §7 — ~30 facts that each cost real time; do not
>    re-derive them
>
> Full hardware evidence, R-0 through R-19:
> `docs/findings/P15-live-iso-recon.md` (raw logs in `P15-recon-raw/`).
>
> ### The single most important open item
>
> **§5.10 — Steam's own "Switch to Desktop" menu item has never been proven.**
> P1.5 exercised the Gaming→Desktop path by invoking our shim directly; Steam's
> Power menu never offered the item (it was stuck in first-run behind §5.14).
> **That is the half of the product's core promise a user actually touches.**
> Resolving it needs the Deck and a real Steam login.
>
> ### Two defects found, deliberately left unfixed — decide before coding
>
...
>
> T4's installer screens and OSK (now re-scoped by §5.9 — lizard mode already
> makes the installer navigable, so the real gap is **text entry**), and T5's
> ISO fork groundwork (`docs/tasks/T5-iso-and-payload.md`), which now has three
> concrete constraints: offline-only pacman, the encryption default, and repo
> precedence.
...
| T3 | `docs/tasks/T3-gaming-mode.md` | Sonnet/Opus | 🟡 **both switch directions proven.** Left: §5.10, §5.11, P2.1–P2.4 |
...
**Phase 1 is closed. Start phase 2.** Two sensible entry points:

- **With the Deck** (operator present): **P2.4 + P2.1** — resolve §5.10
  (Steam's Switch-to-Desktop item, the untested half of the core promise) and
  §5.11 (rotation: the greeter and desktop are both 90° off). Needs a Steam
  login, so the operator has to be there anyway.
- **Without the Deck:** **P2.5** (T4's installer screens — text entry is the
  real gap) or **P2.7** (T5's `omarchy-iso` fork).

...
`docs/START-HERE.md` that's the operator's call. Listed under "Blocked on
human" in `docs/PROGRESS.md`.**

## Why

The naive test loop is `dd`/Etcher-flash a USB (~20 min with verify) →
boot the Deck → find a bug → repeat. `docs/PLAN.md` §9.3 identifies Ventoy as
...
Once the override loader (`test/lib/vm-script-loader.sh` in this repo) lands in
...
a few-KB copy instead of a multi-GB one — see `test/lib/vm-script-loader.sh` for
the loader logic and its own tests for the precedence rules.

## Not yet verified

This finding is written from `docs/PLAN.md` §9.3's reasoning and Ventoy's

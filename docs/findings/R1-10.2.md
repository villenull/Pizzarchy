...
Signal (a) was already killed in `docs/PROGRESS.md` ("Foundational assumption in
...
1. **Build-time — fork `omarchy-iso` only** (per `docs/findings/R1-10.1.md`): Valve
...
- T1 gets a concrete template it didn't have: `install/hardware/intel/ptl-kernel.sh`
  ...
  Read it before writing `src/omarchy-deck-kernel.sh`'s successor.

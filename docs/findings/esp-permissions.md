...
**Status: workaround shipped and in active use in `src/omarchy-deck-kernel.sh`
...
tooling, no `src/omarchy-deck-kernel.sh`, nothing from this project. See
`docs/PROGRESS.md`, Findings section, the paragraph beginning "**PLAN.md §8.5 —
...
now, so `src/omarchy-deck-kernel.sh`'s `stage_esp_permissions` stage rewrites
...
`docs/drafts/upstream-esp-permissions-omarchy.md` — full title, reproduction
...
in `src/omarchy-deck-kernel.sh` should be revisited: the `fmask=0133,dmask=0022`
...
`src/omarchy-deck-kernel.sh` already carries a "SECURITY TRADEOFF — deliberately

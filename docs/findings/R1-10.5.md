...
- **Secure Boot — the load-bearing evidence.** The operator's Deck already
  ...
  install `docs/PROGRESS.md` flags as the project's most valuable test asset —
...
## ⚠️ Correction 2026-08-17 — the button combination above is mislabelled

This finding says the operator entered "the Deck's **Boot Manager** (Vol+ +
Power)". Those two things do not go together, and the label is the part that is
wrong:

* **Volume Down + Power → Boot Manager** (choose which device to boot)
* **Volume Up + Power → BIOS setup** (which *also* lists boot order, which is
  almost certainly why this got recorded as the Boot Manager)

`docs/RECOVERY.md` and `README.md` both say Volume **Down** + Power, in five
places between them, and that is the one to trust: the operator has since
booted the installer from a Ventoy stick repeatedly using exactly that
combination, across several releases. Empirical, repeated, on the target
device.

Nothing in this finding's *conclusion* changes — Limine was the default EFI
entry and no Secure Boot change was needed. Only the parenthetical is wrong.

Raised by an agent that spotted RECOVERY.md and this file disagreeing, and it
was right to raise it: a wrong button combination on the recovery path is
exactly the kind of error that strands somebody. It just resolved in the other
direction.

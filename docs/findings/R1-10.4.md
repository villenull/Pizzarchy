...
1. **Base image.** Official Arch Linux cloud image
   ...
   seed (same FAT-label technique as this repo's `test/lib/vm-cidata.sh`, since there
...
3. **Phase 2 — the actual test.** Rebooted the same disk with **`-nic none`**
   ...
   `test/vm/vm-install-test.sh` does its offline install test), `cloud-init` disabled.
...
3. **Note this is over-determined — and phase 4 proved it.** Even with the
   ...
   packaging strategy. So the honest framing already written in `docs/PLAN.md`
...
4. **The Wi-Fi screen (`docs/PLAN.md` §6.1a item 7) is promoted.** It stays
...
- **Boot order.** Attaching the `cidata` seed as a plain `if=virtio` drive
  ...
  `test/vm/vm-install-test.sh` if it ever grows a second data drive.

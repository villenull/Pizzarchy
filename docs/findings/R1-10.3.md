...
in the same session as the first hardware run of `src/omarchy-deck-kernel.sh`.
...
- `docs/findings/R1-10.4.md` confirmed Steam requires network to sign in on first
  launch — over-determined, not a fixable gap.
- `docs/PLAN.md` §6.1a item 7 requires a **controller-navigable Wi-Fi screen shown
...
- **The mapper itself does not exist.** T2 is not started — the repo has only
  `docs/tasks/T2-gamepad-input-spike.md`. What is verified here is that every

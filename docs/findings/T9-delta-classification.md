...
| 🟠 | `bin/omarchy-provision-owner` | First-boot OOBE screen re-centers vertically off `stty size` and repaints on VT resize; `ttfx --xterm-colors`; the "Starting Omarchy..." card is gone. ⚠️ **Corrected 2026-08-12** — this row previously said the VT is *unrotated*. It is not: we set `fbcon=rotate:1` on the kernel cmdline (`/etc/limine-entry-tool.d/50-deck-fbcon-rotation.conf`, `docs/PROGRESS.md` §5.11), measured live in `/proc/cmdline` and in every `limine.conf` entry (`docs/findings/P22-deck-conformance-sweep.md` §6). **Re-derived:** `stty size` reports the *post*-rotation grid, so the centering math runs on an ordinary landscape console — measured `25 80` installed, `50 160` in the live ISO (§5.11, §7) — **not** on an 800×1280 portrait one, and the original "portrait scanout breaks the centering" worry does not apply. It stays a look-at-a-screen item for T4/T5 for two narrower reasons: the two environments differ 4× in console area, and the installed system's geometry was never measured *before* the rotation landed, so `25 80` is not attributable to it. |

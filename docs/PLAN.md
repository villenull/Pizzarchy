...
>
> **`docs/PROGRESS.md` is authoritative wherever the two disagree.** Do not edit
> this file to "fix" it; record the correction in `docs/PROGRESS.md` instead. The
...
> |---|---|---|
> | §2 grounding facts | `OMARCHY_INSTALLER_REPO`/`_REF` "is the intended extension point" — the vars do not exist | `docs/PROGRESS.md` §3.1 |
> | §2, §5, §6.4 | "Fork `28allday/Super-Shift-S-Omarchy-Deck-Mode`" — renamed twice, superseded by DeckShift, and **unlicensed** (no right to fork) | `docs/PROGRESS.md` §2.4, §3.7 |
> | §3 goal 3, §6.2 | "Installer works fully offline" — **constraint retired**; the install may use Wi-Fi | `docs/PROGRESS.md` §2.2 |
> | §3.1 | "Ship a v0 before the ISO" — **reversed**; the ISO is the deliverable | `docs/PROGRESS.md` §2.1 |
> | §4 architecture diagram | Three-layer design hinges on the nonexistent env vars | `docs/PROGRESS.md` §3.1 |
> | §5 repo plan | Four-repo layout; never created, and its fork bases are wrong | `docs/PROGRESS.md` §3.1 |
> | §6.2 | Lists `gamescope-session-*` as packages to source | **Valve's `gamescope` already ships the entire session** — `docs/PROGRESS.md` §4.1 |
> | §6.3 | Preset-patching automation | Valve ships no preset; UKIs come from `limine-mkinitcpio-hook` — `docs/PROGRESS.md` §7 |
> | §6.4 | "Fork and Deck-ize Super-Shift-S" | This project ships its own switch layer — `docs/PROGRESS.md` §4 |
> | §7 timeline | Week estimates predate all findings | `docs/PROGRESS.md` §1 |
...
- **It de-risks the hard part.** T4 and T5 both depend on assumptions not
  yet verified (`docs/PLAN.md` §10.1, §10.4, and the T2 spike). v0 doesn't.
...
**Result: CONFIRMED, signing caveat KILLED** (`docs/findings/R1-10.1.md`). No
...
upper bound widened. See `docs/findings/R1-10.1.md` for full file/line citations
...
**Result: PARTIAL — right conclusion, wrong mechanism** (`docs/findings/R1-10.2.md`).
...
(`docs/findings/R1-10.3.md`). Design (a): add `steam -silent` to Hyprland autostart
...
**Result: CONFIRMED** (`docs/findings/R1-10.4.md`) — tested for real in a
...
**Result: CONFIRMED** (`docs/findings/R1-10.5.md`, operator-verified on their own
...
**Result: PARTIAL, staged for operator review** (`docs/findings/R1-10.6.md`).

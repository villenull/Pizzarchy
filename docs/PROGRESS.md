...
| 🆕 **extest, both targets** | `~/ISOs/extest-cb77cd4/libextest-{i686,x86_64}.so` | Built 2026-08-11 at pin `cb77cd4` (MIT). ⚠️ **Not "v1.0.4" — corrected 2026-08-16.** Upstream's only tags are `1.0`–`1.0.3`; tag `1.0.3` dereferences to `0d06867`, which is this commit's PARENT. `cb77cd4` is untagged `main` HEAD whose `Cargo.toml` self-declares 1.0.4. Pinning a SHA is fine, but do not call it a release. The i686 one is what T10 preloads into Steam (Steam's input process is 32-bit). Rebuild: `tools/build-extest.sh` — pinned, licence-gated, contained toolchain, needs network once |
...
| 🆕 **OUR 4.0.4 RELEASE ISO — the current one** | `~/.cache/omarchy-deck/iso-build-4.0.4/release/omarchy-2026.09.20-x86_64-quattro.iso`, and on the stick as `/run/media/villenull/Ventoy/pizzarchy-omarchy-4.0.4-2026-09-20-x86_64.iso` | **6,154,983,424 B (5.73 GiB)**, sha256 `2fa7a02031185615f41dd12ec078e136a749edd40a591a6f9930ee1f7d25f814` — **re-read off the stick with `dd iflag=direct` and identical**. Built 2026-09-20, exit 0, all guards green, on the 4.0.4 pins (runtime `omacom/omarchy@c668141e9c42`); payload byte-identical to commit `3236673`. **Supersedes the `e9fbd8ed…` 4.0.0 ISO above. Never booted — see §5.47** |
| 🆕 **Omarchy 4.0.4 ISO (upstream's)** | `/run/media/villenull/Ventoy/omarchy-4.0.4.iso` | 6,185,304,064 B, sha256 `ddeded2758c48318d201dfdac905ecb28f570441883f0c052ea3cd5d05acf92d` — **matches upstream's published 4.0.4 checksum**. The 4.0.4 pins were measured from inside it: `docs/findings/v404-pin.md`. Left untouched by the 2026-09-20 copy |
...
- **R-54:** the operator asked for one last try at higher effort, and it found
  what the first analysis missed — **extest** (MIT, pinned `cb77cd4` — untagged
  upstream HEAD, *not* a v1.0.4 release; see the payload table above),
...
### 5.47 🆕 THE 4.0.4 RELEASE ISO, BUILT FROM THE ACCEPTED TREE (2026-09-20, operator-authorised)

**Authorised after the accepted Deck boot (§5.46):** build the ISO. This is the first
release artefact on the **4.0.4** pins (`docs/findings/v404-pin.md`); every earlier ISO in
§1.1 is a 4.0.0-era artefact, and the 2026-09-17 build that sat in the same cache was
**never approved** — it was moved out of the release directory before this build so the two
cannot be confused. Nothing was relabelled.

| | |
|---|---|
| Artefact | `pizzarchy-omarchy-4.0.4-2026-09-20-x86_64.iso` (built as `~/.cache/omarchy-deck/iso-build-4.0.4/release/omarchy-2026.09.20-x86_64-quattro.iso`) |
| Size / sha256 | 6,154,983,424 B (5.73 GiB) · `2fa7a02031185615f41dd12ec078e136a749edd40a591a6f9930ee1f7d25f814` |
| Build | `iso/bin/build`, exit 0, 3m26s wall, started 2026-09-20 21:41Z (`build5.log`) |
| Pins | iso `omacom/omarchy-iso@7cfb7111` · runtime `omacom/omarchy@c668141e9c42` · pkgs `omacom/omarchy-pkgs@5fe2367` |
| Guards | all green: 6.1 ref/mirror, 6.3 scratch pacman cache, 6.4a (4 orchestrator binaries), 6.4b (`omarchy` + `omarchy-settings` 4.0.4-1, `omarchy-deck` 0.2.0-1), 6.5a/6.5b payload audits, 6.6 (4 runtime patches apply), 6.7 live OSK, 6.8 package lists |
| Payload provenance | airootfs.sfs extracted **from the finished ISO**: `usr/share/omarchy-iso/deck/session/deck-session.sh` md5 `30f01b2093cff26bb31c603fb4ef5fb8` and `usr/local/bin/deck-input-mapper` md5 `1c3d4357b9d6deff213bc0d2191d7c84` (0755) — byte-identical to commit `3236673`, i.e. the tree the operator's accepted boot ran |

**On the stick, and verified by reading it back.** Copied to
`/run/media/villenull/Ventoy/pizzarchy-omarchy-4.0.4-2026-09-20-x86_64.iso` (14m53s,
~6.6 MB/s write; `sync` completed before hashing). The **whole file was then re-read from
the stick with `dd iflag=direct`** — page cache bypassed, 75 s at ~82 MB/s — and its sha256
is `2fa7a020…5f814`, identical to the source. Upstream's `omarchy-4.0.4.iso` is untouched at
the stick's root beside it; nothing else on the USB was changed, no reformat and no eject
(12G used / 104G free afterwards).

⚠️ **What this ISO is NOT.** It has not been booted — not on the Deck, not in QEMU — and no
install has been run from it. The §5.46 acceptance was of a **live Deck** carrying the
deployed helper, not of this image. The guards prove the payload and the pins, nothing about
first boot. The 4.0.4 rolling-mirror caveat also still applies: the exact package set is
whatever the mirrors served at 21:41Z, so a rebuild is not expected to be bit-identical.

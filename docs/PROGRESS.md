...
| **T4** Controller-only installer | 🟡 **`deck-form.sh` has a route onto the ISO (session 22)** — the 5th overlay patch, argued in its own header, plus `deck-dashboard.sh` (T4a, session 22). Screen spec `docs/tasks/T4-screen-spec.md` still the design authority. `--osk-start-shown` + the mapper's `bound` readiness marker exist now, so S1's Wi-Fi passphrase prompt no longer degrades by construction. **Left:** P2.5/P2.6, the QEMU install run itself — needs a real ISO, see T5 |
| **T5** ISO + package payload | 🟡 **T5a–T5f all landed (session 22).** Parity **proven** (`T5a-parity.md` Appendix A, all four tests pass), the `omarchy-deck` pacman package ships T12's real payload, five overlay patches (budget raised 4→5, argued each time), guard 6.4a **and 6.4b** now understand our own package as a provider, all four rotation surfaces baked in. 🔴 **The one thing left, and it gates criterion 1 and criterion 4 both: `iso/bin/build` has never been run.** Not a code gap — every guard, every step, every patch is unit- and mutation-tested against the patched tree; nobody has executed a real container build. `input.lua` (OSK XKB + `above_lock`) still has no writer |
